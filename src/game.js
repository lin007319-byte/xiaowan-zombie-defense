(function () {
  ...
  function shoot(p,scale=1) {
    ...
    const isStar=p.baseId==="star",isGloom=p.baseId==="gloom",radial=isStar||isGloom||p.genes.includes("radial");
    const angles=isStar
      ? Array.from({length:5},(_,i)=>-Math.PI/2+i*TAU/5)
      : isGloom
        ? Array.from({length:8},(_,i)=>-Math.PI/8+i*TAU/8)
        : radial
          ? [-.42,-.21,0,.21,.42]
          : Array.from({length:count},(_,i)=>(i-(count-1)/2)*.035);
    for(let i=0;i<angles.length;i++){
      const a=angles[i],speed=isGloom?235:radial?305:290+i*14,ignited=p.genes.includes("ignite"),tipRadius=isStar?38:isGloom?34:0;
      state.bullets.push({
        ...
        kind:isStar?"star":isGloom?"gloom":radial?"star":p.baseId,
        rangeBounds:isGloom?{minX:c.x-GRID.cw*1.5,maxX:c.x+GRID.cw*1.5,minY:c.y-GRID.ch*1.5,maxY:c.y+GRID.ch*1.5}:null,
  ...
  function drawRadialPlant(p,x,y,alpha,name,sub,attack=0){
    ...
    if(!isStar){ctx.fillStyle="#d9bbff";for(let i=0;i<8;i++){const a=start+i*TAU/8;ctx.beginPath();ctx.arc(Math.cos(a)*35,Math.sin(a)*35,3.5,0,TAU);ctx.fill();}}
