(function () {
  ...
  function drawPlacementPreview(){if(state.dragging||!state.selected||!state.hoverCell||plantAt(state.hoverCell.row,state.hoverCell.col))return;const def=Core.PLANTS[state.selected],c=cellCenter(state.hoverCell.row,state.hoverCell.col);drawTextEntity(`我是${def.short}`,c.x,c.y,def.color,.46,"种植预览",92);}
