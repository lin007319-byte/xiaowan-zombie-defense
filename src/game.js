(function () {
  ...
  const W = 1280, H = 720;
  const GRID = { x: 264, y: 84, cols: 9, rows: 5, cw: 96, ch: 120 };
  const CARD_Y = 22;
  ...
  const CARD_SIGILS={pea:"●",sun:"✹",cherry:"✦",nut:"⬢",potato:"◆",frost:"❄",chomper:"◇",gloom:"◉",puff:"·",sunshroom:"☀",fume:"◒",hypno:"◎",iceShroom:"❅",doom:"☢",lilypad:"⌁",pepper:"♨",spikeweed:"✷",torchwood:"♨",tallnut:"▥",lantern:"☀",cactus:"✣",blover:"✤",star:"★",pumpkin:"⬡",magnet:"∩",cabbage:"●",corn:"▦",garlic:"◈",umbrella:"☂",melon:"◉"};
  ...
  function shoot(p,scale=1) {
    ...
    const radial=p.baseId==="star"||p.baseId==="gloom"||p.genes.includes("radial"),angles=radial?[-.42,-.21,0,.21,.42]:Array.from({length:count},(_,i)=>(i-(count-1)/2)*.035);
    for(let i=0;i<angles.length;i++){const a=angles[i],speed=radial?305:290+i*14,ignited=p.genes.includes("ignite");state.bullets.push({x:c.x+26-i*3,y:c.y-9+(radial?0:i*7),row:p.row,vx:Math.cos(a)*speed,vy:Math.sin(a)*speed,spin:Math.random()*TAU,kind:radial?"star":p.baseId,damage:Math.max(8,Core.damageFor(p)*(i&& !radial?0.62:1)*scale*(ignited?1.35:1)),frost:p.baseId==="frost"||p.genes.includes("frost")||p.genes.includes("deepfreeze"),deepfreeze:p.genes.includes("deepfreeze"),sunny:p.genes.includes("producer"),burst:(p.genes.includes("burst")&&p.attackCount%6===0)||(p.genes.includes("nova")&&p.attackCount%10===0),stun:p.baseId==="corn"||p.genes.includes("stun"),fire:p.genes.includes("fire")||ignited,gust:p.genes.includes("gust"),crit:p.baseId==="cactus"||p.genes.includes("crit"),light:p.baseId==="lantern"||p.genes.includes("reveal"),magnet:p.genes.includes("magnet"),splash:p.baseId==="melon"||p.baseId==="cabbage"||p.genes.includes("splash"),hitsLeft:["puff","fume","gloom"].includes(p.baseId)||p.genes.includes("pierce")?2:1,hitIds:[],alive:true,life:4,color:ignited?"#ff9148":def.color});}
    sfx("shoot");
  }
  function produce(p) {
  ...
  function drawGrid(){
    const c=state.hoverCell;if(c&&state.selected&&!plantAt(c.row,c.col)){const p=cellCenter(c.row,c.col),x=p.x-GRID.cw/2+5,y=p.y-GRID.ch/2+5;ctx.strokeStyle="#f8df78";ctx.lineWidth=3;roundRect(x,y,GRID.cw-10,GRID.ch-10,12);ctx.stroke();}if(state.dragCell){const p=cellCenter(state.dragCell.row,state.dragCell.col),x=p.x-GRID.cw/2+5,y=p.y-GRID.ch/2+5;ctx.fillStyle="rgba(181,244,145,.12)";roundRect(x,y,GRID.cw-10,GRID.ch-10,12);ctx.fill();ctx.strokeStyle="#b9f59b";ctx.lineWidth=3;roundRect(x,y,GRID.cw-10,GRID.ch-10,12);ctx.stroke();}
  }
  ...
  function drawZombie(z){
    const vaultLift=z.vaultAnim>0?Math.sin((1-z.vaultAnim/.72)*Math.PI)*72:0,airLift=z.air&&z.grounded<=0?18+Math.sin(state.time*4+z.id)*3:0,walk=Math.sin(z.step)*2,status=[z.burn>0?"燃烧":"",z.stun>0?"眩晕":"",z.slow>0?"减速":"",z.metalStripped?"破甲":""].filter(Boolean).join(" · ");
    drawTextEntity(ZOMBIE_TEXT[z.kind]||"我是僵尸",z.x,z.y-vaultLift-airLift+walk,z.hit>0?"#ffffff":z.color,1,status,z.kind==="giant"?84:76);drawEnemyHp(z);
  }
  ...
  window.__gardenDebug={
    ...
    snapshot:()=>({mode:state.mode,gameMode:state.gameMode,loadout:[...state.loadout],wave:state.wave,fps:state.fps,timeStop:state.timeStop,timeScale:state.timeScale,sun:state.sun,selected:state.selected,mowers:[...state.mowers],plants:state.plants.filter(p=>p.alive).map(p=>({id:p.baseId,row:p.row,col:p.col,rank:p.rank,genes:p.genes,materials:p.materialIds||[],name:p.displayName})),zombies:state.zombies.filter(z=>z.alive).length,zombieKinds:[...new Set(state.zombies.filter(z=>z.alive).map(z=>z.kind))],zombieKindsSeen:[...state.zombieKindsSeen],stats:{...state.stats,discovered:[...state.stats.discovered]}})
  };
