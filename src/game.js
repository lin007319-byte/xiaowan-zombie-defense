(function () {
  ...
  function updateGloom(p,dt,hasEnemy){
    ...
    for(const z of targets){z.hp-=damage;z.hit=1;z.weaken=Math.max(z.weaken,.18);if(z.hp<=0)killZombie(z);}
    state.effects.push({type:"gloomArea",x:c.x,y:c.y,w:GRID.cw*3,h:GRID.ch*3,life:.32,max:.32,color:"#b892ef"});
    burstParticles(c.x,c.y,"#c9a8ff",18,105);tone(145,.08,"sine",.02,55);
    p.gloomShotsLeft--;
